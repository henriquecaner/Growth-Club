...
**Current date context:** estamos em 2026-04-22. Barte S1E1 é em jun/2026 (~6 semanas).

## Repository status

This is **not** a greenfield scaffold. The project is mid-execution on the **Marca** phase. What exists:

- ✅ **Business Plan v1.2 approved** — `docs/superpowers/specs/2026-04-22-growth-club-business-plan-design.md` (ADR-001 em `.specs/project/STATE.md`)
...
- ⏳ **Chunk 5** (brand book consolidation) — pending
- ⏳ **Site** (capture form + thank-you at `growthclub.pro`) — specs not written yet; separate sub-project after Marca
...
5. **Pricing (Fase 1):** `Growth Hacker` (free, 12 meses de lock inicial pros primeiros 1.000). `Growth Hacker Master` R$ 990/ano (com tier early de R$ 690 para primeiros 100-200 pagantes). `Founder Member` R$ 2.079 (3 anos com 30% desc.), quota 100 = 25 em Barte S1E1 + 75 pós-checklist.
6. **Livecast patrocinado entra no MVP** (amigo com agência patrocina).
...
- `STATE.md` — **append-only** log of ADRs, blockers, risks, lessons. Currently: **ADR-001** (Business Plan v1.2 aprovado) + **L-001** (ativos orgânicos preexistentes). Never overwrite past entries.
...
1. **User polishing Chunks 1–3 drafts** (decisions, visual, voice) — don't rewrite unless asked; collaborate on edits.
2. **Task 1.4 in plan** (marcas-filhas / sub-grupos WhatsApp) — blocked on user input about 4 sub-groups; don't invent.
3. **Chunk 2 logo steps 1c–1e** — user executes in Figma; Claude helps with sketches/feedback, not the vector.
4. **Chunk 4 (templates)** — depends on logo locked.
5. **Chunk 5 (brand book)** — depends on Chunks 2–4 done.
6. **Legal/operational §11 of business plan** (CNPJ, regime tributário, revshare contracts) — critical before first Founder Member signs $.
7. **Site sub-project** — separate spec + plan, starts after Marca closes.
